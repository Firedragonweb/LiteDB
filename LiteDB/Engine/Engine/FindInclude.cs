...
namespace LiteDB
{
    public partial class LiteEngine
    {
        ...
        /// </summary>
        public IEnumerable<BsonDocument> Find(string collection, Query query, string[] includes, int skip = 0, int limit = int.MaxValue, bool shallowMode = false)
        {
            if (includes == null) throw new ArgumentNullException(nameof(includes));

            var docs = this.Find(collection, query, skip, limit, shallowMode);
